...
Untrusted input is a batch; it can only ever become one of a fixed set of decisions over a
...
- **Untrusted input:** `fetch<web>` — a batch
- **Closed type:** `type Decision = GateQual(QualityTier) | Quarantine` where `QualityTier` is closed
- **Sink / capability:** `grant gateQuality confidence 70` → `privileged { gateQuality(d) }`
- **Consequence axes:** Trust, Confidence

## Run the demo

```sh
examples/apps/quality-gate/demo.sh
```
...
- `quality-gate_safe.wave` — the correct design.
- `quality-gate_unsafe.wave` — the tempting-but-wrong version (the negative example a model must learn to reject).
